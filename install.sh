...
install() {
    ...
    echo "  sudo systemctl start wg-failover@<peer-ip>.service"
    echo ""
    echo -e "${YELLOW}To enable automatic startup on boot:${NC}"
    echo "  sudo systemctl enable wg-failover@<peer-ip>.service"
    echo ""
    echo -e "${YELLOW}Replace <peer-ip> with your WireGuard peer's IP address${NC}"
    echo ""
    echo -e "${YELLOW}To check service status:${NC}"
    echo "  sudo systemctl status wg-failover@<peer-ip>.service"
    echo ""
    echo -e "${YELLOW}To view logs:${NC}"
    echo "  sudo journalctl -u wg-failover@<peer-ip>.service -f"
    echo "  or"
    echo "  sudo tail -f $LOG_PATH"
    
    echo ""
    echo -e "${GREEN}Configuration file location: $CONFIG_PATH${NC}"
    echo -e "${GREEN}You must edit this file to set your network interfaces and WireGuard peer IP${NC}"
}
